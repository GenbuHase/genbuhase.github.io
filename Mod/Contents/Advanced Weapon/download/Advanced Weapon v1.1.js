...
const AdvancedWeapon = (function () {
	...
	function AdvancedWeapon () {
		...
		loadWeapon: {
			value () {
				for (let Type in PATH.Low) {
					let reader = new AdvancedWeapon.JsLoaderFromTexturePack(PATH.Low[Type]);
		...
		loadLang: {
			value () {
				let reader = new AdvancedWeapon.JsonLoaderFromTexturePack(PATH.Lang);
		...
		addAxe: {
			value (axe) {
				if (axe instanceof AdvancedWeapon.Axe) {
					...
					throw new TypeError("The parameter 1 isn't " + axe.constructor.name + ", it's " + AdvancedWeapon.Axe.name + ".");
				}
			}
		}
	});
	...
	AdvancedWeapon.JsonLoaderFromTexturePack = (function () {
	...
	AdvancedWeapon.JsLoaderFromTexturePack = (function () {
