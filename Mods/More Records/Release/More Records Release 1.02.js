	...
	function newLevel() {
		...
		if (!File.IsVaild(File.SpecialFolder.MCPE + "More Records/Records.Json")) {
			...
			for (var i = 0; i < RecordFile.length; i++) {
				...
				Player.addItemCreativeInv(Records[i][0], 1, 0);
			}
		}

		if (File.IsVaild(File.SpecialFolder.MCPE + "More Records/States.Sav")) {
			JukeBoxes[0] = File.ReadFile(File.SpecialFolder.MCPE + "More Records/States.Sav").split(",")[0];
			JukeBoxes[1] = File.ReadFile(File.SpecialFolder.MCPE + "More Records/States.Sav").split(",")[1];
			JukeBoxes[2] = File.ReadFile(File.SpecialFolder.MCPE + "More Records/States.Sav").split(",")[2];
			JukeBoxes[3] = File.ReadFile(File.SpecialFolder.MCPE + "More Records/States.Sav").split(",")[3];
		}
	}

	function leaveGame() {
		if (JukeBoxes != []) {
			File.CreateFile(File.SpecialFolder.MCPE + "More Records/States.Sav");
			File.WriteFile(File.SpecialFolder.MCPE + "More Records/States.Sav", JukeBoxes[0] + "," + JukeBoxes[1] + "," + JukeBoxes[2] + "," + JukeBoxes[3]);
		...
		if (BlockID == 250 && BlockDamage == 0) {
			for (var i = 0; i < Records.length; i++) {
				if (ItemID == Records[i][0]) {
					...
					JukeBoxes = [X, Y, Z, ItemID];
				}
			}
		} else if (BlockID == 250 && BlockDamage == 1) {
			Actions.StopMusic();
			Level.dropItem(X, Y + 1, Z, 0.5, JukeBoxes[3], 1, 0);
