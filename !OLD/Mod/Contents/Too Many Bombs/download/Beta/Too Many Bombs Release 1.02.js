	...
	switch(getCarriedItem()) {
		...
		case Bomb.UltimateBomb:
			...
			for(var i = 0; i < EntityAll.length; i++) {
				if(Entity.getEntityTypeId(EntityAll[i]) == 81) {
					if(Level.getTile(Math.round(Entity.getX(EntityAll[i])), Math.round(Entity.getY(EntityAll[i])) - 1, Math.round(Entity.getZ(EntityAll[i]))) != 0) {
						explode(Math.round(Entity.getX(EntityAll[i])) + new java.util.Random().nextInt(10), Math.round(Entity.getY(EntityAll[i])), Math.round(Entity.getZ(EntityAll[i])) + new java.util.Random().nextInt(10), 5.0);
						explode(Math.round(Entity.getX(EntityAll[i])) + new java.util.Random().nextInt(10), Math.round(Entity.getY(EntityAll[i])), Math.round(Entity.getZ(EntityAll[i])) + new java.util.Random().nextInt(10), 5.0);
						explode(Math.round(Entity.getX(EntityAll[i])) + new java.util.Random().nextInt(10), Math.round(Entity.getY(EntityAll[i])), Math.round(Entity.getZ(EntityAll[i])) + new java.util.Random().nextInt(10), 5.0);
						explode(Math.round(Entity.getX(EntityAll[i])) + new java.util.Random().nextInt(10), Math.round(Entity.getY(EntityAll[i])), Math.round(Entity.getZ(EntityAll[i])) + new java.util.Random().nextInt(10), 5.0);
						explode(Math.round(Entity.getX(EntityAll[i])) + new java.util.Random().nextInt(10), Math.round(Entity.getY(EntityAll[i])), Math.round(Entity.getZ(EntityAll[i])) + new java.util.Random().nextInt(10), 5.0);
...
function AddItems() {
	...
	ModPE.setItem(Bomb.GrandBomb, "StandardBomb", 0, "土の爆弾", 1);
}

function AddRecipes() {
	Item.addShapedRecipe(Bomb.Bomb, 3, 0, ["GSG", "GGG", "GGG"], ["G", Materials.GunPowder, 0, "S", Materials.String, 0]); //普通の爆弾 のレシピ追加(火薬 * 8 & 糸 * 1)
}

function SetCategory() {
	Item.setCategory(Bomb.Bomb, Category.Tool);
	Item.setCategory(Bomb.SuperBomb, Category.Tool);
	Item.setCategory(Bomb.HyperBomb, Category.Tool);
	Item.setCategory(Bomb.UltimateBomb, Category.Tool);
	...
	Item.setCategory(Bomb.FireBomb, Category.Tool);
	Item.setCategory(Bomb.SuperFireBomb, Category.Tool);
	Item.setCategory(Bomb.HyperFireBomb, Category.Tool);
	Item.setCategory(Bomb.UltimateFireBomb, Category.Tool);
