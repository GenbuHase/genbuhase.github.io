/*/
 *#######################################################
 *Villager's Inventory Release 1.2
...
with (new DataHelper()) {
	...
	Player.prototype.HavingItems = function (ID, Damage) {
		var Count = 0;
	...
	function deathHook(Attacker, Victim) {
		for (var i = 0; i < Villagers.length; i++) {
			if (Villagers[i]["UUID"] == Victim) {
				for (var j = 0; j < 5; j++) {
					Level.dropItem(Entity.getX(Victim), Entity.getY(Victim), Entity.getZ(Victim), 1, Villagers[i]["Inventory"][a]["ID"], Villagers[i]["Inventory"][a]["Amount"], Villagers[i]["Inventory"][a]["Damage"]);
				}
