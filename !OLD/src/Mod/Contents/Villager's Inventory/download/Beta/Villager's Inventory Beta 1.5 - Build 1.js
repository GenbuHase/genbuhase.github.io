...
var NativeClassExtender = function () {
	this.Enable = function () {
		...
		Entity.GetDirection = function (Ent) {
...
with (new DataHelper()) {
	...
	function newLevel() {
		...
		ModPE.setItem(IDs.VillagersRod, "blaze_rod", 0, "Villager's Rod", 1);
		Player.addItemCreativeInv(IDs.VillagersRod, 1, 0);
		Item.addShapedRecipe(IDs.VillagersRod, 0, 1, ["NHN", "NRN", "NRN"], ["N", 0, 0, "H", 397, 3, "R", 368, 0]);
	}
	...
	function modTick() {
		if (getCarriedItem() == 388) {
			for (var i = 0; i < Villagers.length; i++) {
				if (Math.abs(Entity.getX(Player.getEntity()) - Entity.getX(Villagers[i].UUID)) < 10) {
					var Pos = 
						Entity.GetDirection(Player.getEntity()) == "南" ? [0, -1] : 
						Entity.GetDirection(Player.getEntity()) == "南南西" || Entity.GetDirection(Player.getEntity()) == "南西" || Entity.GetDirection(Player.getEntity()) == "西南西" ? [1, -1] : 
						Entity.GetDirection(Player.getEntity()) == "西" ? [1, 0] : 
						Entity.GetDirection(Player.getEntity()) == "西北西" || Entity.GetDirection(Player.getEntity()) == "北西" || Entity.GetDirection(Player.getEntity()) == "北北西" ? [1, 1] : 
						Entity.GetDirection(Player.getEntity()) == "北" ? [0, 1] : 
						Entity.GetDirection(Player.getEntity()) == "北北東" || Entity.GetDirection(Player.getEntity()) == "北東" || Entity.GetDirection(Player.getEntity()) == "東北東" ? [-1, 1] : 
						Entity.GetDirection(Player.getEntity()) == "東" ? [-1, 0] : 
						Entity.GetDirection(Player.getEntity()) == "東南東" || Entity.GetDirection(Player.getEntity()) == "南東" || Entity.GetDirection(Player.getEntity()) == "南南東" ? [-1, -1] : [5, 5];

					Entity.setVelX(Villagers[i].UUID, Entity.getX(Player.getEntity()) - Entity.getX(Villagers[i].UUID) + Pos[0]);
					Entity.setVelY(Villagers[i].UUID, Entity.getY(Player.getEntity()) - Entity.getY(Villagers[i].UUID) - 2);
					Entity.setVelZ(Villagers[i].UUID, Entity.getZ(Player.getEntity()) - Entity.getZ(Villagers[i].UUID) + Pos[1]);
				}
			}
		}
	}
						...
						with (GUIHelper.Property) {
							...
							CreateGUI(function () {
											...
											ShowDialog(0, "アイテムメニュー(プレイヤー)",
												AddLinearLayout(2, "Vertical", [
													...
													AddButton(1, "村人に渡す", 10, Color.Magenta, function () {
														if ((VillagerItems[0][0] == 0 && VillagerItems[0][1] == 0) || (VillagerItems[0][0] == MPIDSelect && VillagerItems[0][1] == MPDamageSelect)) {
															for (var i = 0; i < Villagers.length; i++) {
																if (Villagers[i]["UUID"] == Victim && Player.GetItemAmount(MPIDSelect, MPDamageSelect) != 0) {
														...
														} else if ((VillagerItems[1][0] == 0 && VillagerItems[1][1] == 0) || (VillagerItems[1][0] == MPIDSelect && VillagerItems[1][1] == MPDamageSelect)) {
															for (var i = 0; i < Villagers.length; i++) {
																if (Villagers[i]["UUID"] == Victim && Player.GetItemAmount(MPIDSelect, MPDamageSelect) != 0) {
														...
														} else if ((VillagerItems[2][0] == 0 && VillagerItems[2][1] == 0) || (VillagerItems[2][0] == MPIDSelect && VillagerItems[2][1] == MPDamageSelect)) {
															for (var i = 0; i < Villagers.length; i++) {
																if (Villagers[i]["UUID"] == Victim && Player.GetItemAmount(MPIDSelect, MPDamageSelect) != 0) {
														...
														} else if ((VillagerItems[3][0] == 0 && VillagerItems[3][1] == 0) || (VillagerItems[3][0] == MPIDSelect && VillagerItems[3][1] == MPDamageSelect)) {
															for (var i = 0; i < Villagers.length; i++) {
																if (Villagers[i]["UUID"] == Victim && Player.GetItemAmount(MPIDSelect, MPDamageSelect) != 0) {
														...
														} else if ((VillagerItems[4][0] == 0 && VillagerItems[4][1] == 0) || (VillagerItems[4][0] == MPIDSelect && VillagerItems[4][1] == MPDamageSelect)) {
															for (var i = 0; i < Villagers.length; i++) {
																if (Villagers[i]["UUID"] == Victim && Player.GetItemAmount(MPIDSelect, MPDamageSelect) != 0) {
								...
								try {
									for (var i = 0; i < Villagers.length; i++) {
										if (Villagers[i]["UUID"] == Victim) {
											var ItemImg = "resourcepacks/vanilla/images/items/";
											...
											clientMessage(ItemImg + Item.getName(Villagers[i]["Inventory"][0]["ID"], Villagers[i]["Inventory"][0]["Damage"], true) + ".png");
											clientMessage(ItemImg + Item.getName(Villagers[i]["Inventory"][1]["ID"], Villagers[i]["Inventory"][1]["Damage"], true) + ".png");
											clientMessage(ItemImg + Item.getName(Villagers[i]["Inventory"][2]["ID"], Villagers[i]["Inventory"][2]["Damage"], true) + ".png");
											clientMessage(ItemImg + Item.getName(Villagers[i]["Inventory"][3]["ID"], Villagers[i]["Inventory"][3]["Damage"], true) + ".png");
											clientMessage(ItemImg + Item.getName(Villagers[i]["Inventory"][4]["ID"], Villagers[i]["Inventory"][4]["Damage"], true) + ".png");
											
											Items[0] = Villagers[i]["Inventory"][0]["ID"] > 255 ? 
												AddImageView(0, "Bitmap", android.graphics.BitmapFactory.decodeStream(ModPE.openInputStreamFromTexturePack(ItemImg + Item.getName(Villagers[i]["Inventory"][0]["ID"], Villagers[i]["Inventory"][0]["Damage"], true) + ".png")), ScaleType.Normal) : 
												AddImageView(0, "Base64", Images.Slot, ScaleType.Normal);
												
											Items[1] = Villagers[i]["Inventory"][1]["ID"] > 255 ? 
												AddImageView(1, "Bitmap", android.graphics.BitmapFactory.decodeStream(ModPE.openInputStreamFromTexturePack(ItemImg + Item.getName(Villagers[i]["Inventory"][1]["ID"], Villagers[i]["Inventory"][1]["Damage"], true) + ".png")), ScaleType.Normal) : 
												AddImageView(1, "Base64", Images.Slot, ScaleType.Normal);
												
											Items[2] = Villagers[i]["Inventory"][2]["ID"] > 255 ? 
												AddImageView(2, "Bitmap", android.graphics.BitmapFactory.decodeStream(ModPE.openInputStreamFromTexturePack(ItemImg + Item.getName(Villagers[i]["Inventory"][2]["ID"], Villagers[i]["Inventory"][2]["Damage"], true) + ".png")), ScaleType.Normal) : 
												AddImageView(2, "Base64", Images.Slot, ScaleType.Normal);
												
											Items[3] = Villagers[i]["Inventory"][3]["ID"] > 255 ? 
												AddImageView(3, "Bitmap", android.graphics.BitmapFactory.decodeStream(ModPE.openInputStreamFromTexturePack(ItemImg + Item.getName(Villagers[i]["Inventory"][3]["ID"], Villagers[i]["Inventory"][3]["Damage"], true) + ".png")), ScaleType.Normal) : 
												AddImageView(3, "Base64", Images.Slot, ScaleType.Normal);
												
											Items[4] = Villagers[i]["Inventory"][4]["ID"] > 255 ? 
												AddImageView(4, "Bitmap", android.graphics.BitmapFactory.decodeStream(ModPE.openInputStreamFromTexturePack(ItemImg + Item.getName(Villagers[i]["Inventory"][4]["ID"], Villagers[i]["Inventory"][4]["Damage"], true) + ".png")), ScaleType.Normal) : 
												AddImageView(4, "Base64", Images.Slot, ScaleType.Normal);
											...
											, 400, 80, Align.Center, Align.Center, 0, 0);
										}
									}
								} catch (Error) {
									clientMessage(Error);
	...
	function deathHook(Attacker, Victim) {
		...
		if (Entity.getEntityTypeId(Victim) == EntityType.ZOMBIE_VILLAGER) {
			Level.dropItem(Entity.getX(Victim), Entity.getY(Victim), Entity.getZ(Victim), 1, 397, 1, 3);
		}
	}
