<template>
	<Main>
		...
		<img Class = "responsive-img materialboxed" :src = "product.preview" Data-Caption = "ブログ形式サイトのための記事編集ツール">
...
<script>
	export default {
		...
		computed: {
			product () {
				const product = this.products.find(item => item.path === this.$route.params.pathMatch);
				return product || {};
			}
		}
	};
